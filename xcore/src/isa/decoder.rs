    ...
    pub fn new(table: &Table, error_stack: &mut Vec<String>) -> Self {
        ...
        for value in mapping_names {
            if let Some(mapping_name) = value.as_str() {
                ...
                error_stack.push(format!(
                    "value of array entry in 'mappings.names' is of type '{}' instead of type \
                     'string'",
                    value.type_str()
                ));
            }
        ...
        .filter_map(|x| {
            ...
            if parr.len() < 3 || parr.len() > 4 {
                error_stack.push(format!(
                    "expected length of part {parr:?} to be 3 or 4, in the form of [name: string, \
                     bitwidth: integer, type: string, (format: string = \"decimal\")]"
                ));
                None
            } else {
            ...
            for (repr_name, repr) in &fmt.repr {
                ...
                while idx < repr.len() && repr[idx..].contains('%') {
                    ...
                    if !repr[begin..].contains('%') {
                        ...
                        if nmatches == 0 {
                            error_stack.push(format!(
                                "format of {fmt_name}.{repr_name} is trying to reference \
                                 nonexistant slice {var_name}"
                            ));
                        }
                        idx = end + 1;
                    }
...
impl InstructionSlice {
    pub fn new(
        ...
        if !parts.contains_key(&name) {
            error_stack.push(format!(
                "instruction slice with name \"{name}\" referenced in types.{type_name} not \
                 defined in formats.parts"
            ));
        }
        let slice_top =
        ...
        if slice_top > bit_width || slice_bottom > bit_width {
            error_stack.push(format!(
                "instruction slice \"{}\" of type \"{}\" at position {} downto {} is out of range \
                 for bit width {}",
                name,
                type_name,
                slice_top - 1,
