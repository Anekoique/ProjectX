...
Round 03 is the final pre-implementation pass. Round 02 converged at
Ready=Yes (0 CRITICAL / 0 HIGH); the four residuals from
`02_REVIEW.md` (1 MEDIUM + 3 LOW) are absorbed. **R-020** (MEDIUM,
paddr-threading API gap) adopts TR-8 option (b): `last_store` moves
*inside* `Hart::checked_write` itself — one line, no signature
change, I-8 becomes a post-condition of `checked_write` rather than
a caller discipline. **R-022** (LOW) corrects PLIC test count 13 → 14
(verified `grep -c '#\[test\]' plic.rs == 14`). **R-023** (LOW)
rewrites step 18's `X_HARTS` snippet as plain `match` per
`main.rs:45-53`. **R-024** (LOW) unifies Acceptance Mapping C-7 row
to "`≤ 700 lines`". **R-021** is subsumed by R-023's rewrite. No
structural changes to Goals / Architecture / Invariants / Phases;
test arithmetic unchanged (PR1 354 → 367; PR2a 367 → 368; PR2b 368
→ 371). Implementation-ready.

## Log

[**Feature Introduce**] I-8 hook *callee-record refinement* (TR-8 /
R-020): `self.last_store = Some((pa, size))` moves from
`Hart::store` / `Hart::amo_store` into `Hart::checked_write` itself,
gated on `op == MemOp::Store || op == MemOp::Amo`. `pa` is already
in scope as the local returned by `access_bus` at `mm.rs:272`;
recording it before `Ok(())` is one line. `checked_write`'s
signature stays `XResult` — no `XResult<usize>` widening.

[**Review Adjustments**] R-020 (MEDIUM) and R-021..R-024 (LOW)
absorbed via the Response Matrix below; prior rounds
(R-001..R-019, TR-1..TR-7, inherited MASTER) collapse into one row.

[**Master Compliance**] No `03_MASTER.md` (auto-loop). Inherited
directives (00-M-001/002, 01-M-001..004) still apply; enumerated in the Response Matrix.

### Changes from Previous Round

[**Added**] I-8 callee-record body inside `Hart::checked_write` (one
line, gated on `op ∈ {Store, Amo}`) under Architecture / I-8 / API
Surface / step 12.

[**Changed**] "13 existing PLIC tests" → "14" in G-8, step 14,
V-IT-6 (R-022); step 18 `X_HARTS` parse rewritten as `match` per
`main.rs:45-53` (R-023 / R-021); Acceptance Mapping C-7 row reads
"`≤ 700-line budget`" (R-024).

[**Removed**] Round-02 step 12 prose asserting "mm.rs threads the
translated paddr through `MemOp::Store` / `MemOp::Amo` bookkeeping"
— factually wrong about the existing code, replaced with the
callee-record description.

[**Unresolved**] None. Round 02 closed at Ready=Yes; round 03
absorbs the four residuals.

### Response Matrix

| Source | ID | Decision | Resolution |
|--------|----|----------|------------|
| Review | R-020 | Accepted (option b / TR-8) | I-8 record relocates from `Hart::store` / `Hart::amo_store` into `Hart::checked_write` itself, keyed on `op` being a store. No `checked_write` signature change. See I-8, API Surface (mm.rs block), Phase-1 step 12. |
| Review | R-021 | Accepted | Subsumed by R-023's rewrite — the closure-with-inner-`?` shape is gone, replaced by a plain `match`. See Phase-2b step 18. |
| Review | R-022 | Accepted | "13 existing PLIC tests" → "14" in G-8, step 14 prose, V-IT-6, and any other mention. Verified `grep -c '#\[test\]' plic.rs == 14`. |
| Review | R-023 | Accepted | Step 18's `X_HARTS` parse rewritten as `match` mirroring `main.rs:45-53`. See Phase-2b step 18. |
| Review | R-024 | Accepted | Acceptance Mapping row for C-7 reads "`≤ 700-line budget`" matching C-7 prose. Summary text also says 700. |
| Review | TR-8 | Adopted | Callee-record per R-020 above. |
| — | Prior rounds | Carried | R-001..R-019 + TR-1..TR-7 resolved in `02_PLAN.md` Response Matrix. No regression in round 03. |
| Master | 00-M-001 (inherited) | Applied | `Hart` stays arch-internal concrete; no `Arch` trait introduced. |
| Master | 00-M-002 (inherited) | Applied | `hart.rs` lives at `arch/riscv/cpu/`, topic-organised. |
| Master | 01-M-001 (inherited) | Applied | No `selected` identifier; `current` / `current_mut` only. |
| Master | 01-M-002 (inherited) | Applied | Plan body ≤ 700 lines (target met by construction; round 03 trims further). |
| Master | 01-M-003 (inherited) | Applied | No new arch-validity cfg checks. |
| Master | 01-M-004 (inherited) | Applied | Per-hart state lives exclusively under `arch/riscv/cpu/`; `cpu` / `device` / `isa` top levels remain trait APIs + tiny cfg. |
...
- **G-3** Extend `Mswi`, `Mtimer`, `Sswi` to per-hart `Vec` state
  with spec-mandated strides; single-hart MMIO byte-identical.
...
- **G-8** PR2a ships PLIC runtime-size conversion at `num_harts = 1`:
  zero guest-observable change; **14 existing PLIC tests** *(R-022)*
  pass unchanged (re-exercised, not re-counted); 1 new PR2a test
...
- **G-10** Cross-hart LR/SC correctness (I-8) via mm-layer hook:
  ...
  within the granule. Hook is a post-condition of
  `Hart::checked_write` *(R-020)*.
...
- **NG-6** Multi-hart debugger UX. At `num_harts > 1`, `DebugOps`
  targets the hart at `self.current` at call time; per-hart
  selection deferred to a future `xdb-smp-ux` task.
...
I-8 hook *(R-020 / TR-8)* lives inside `Hart::checked_write`:

```rust
fn checked_write(&mut self, addr: VirtAddr, size: usize,
...
Cross-hart reservation invalidation (range-overlap with granule
alignment per R-016):
...
`RVCore::step`, after `step_one`, iff `last_store.take()` returned
`Some`.
...
- **I-8** *(R-011 + R-020)* `Hart::checked_write` post-condition:
  on `Ok`, if `op ∈ {Store, Amo}` then `self.last_store ==
  Some((pa, size))`. `RVCore::step` consumes via `.take()` after
  `step_one` and, if `Some`, calls
  `invalidate_reservations_except(src, addr, size)` — clears
  `harts[h].reservation` for every `h != src` whose granule-aligned
  range overlaps `[addr, addr + size)` with
  `G = RESERVATION_GRANULE`. Covers `store_op`, `fstore_op`, `sc_w`,
  `sc_d`, all 18 AMO variants; future store opcodes inherit by
  construction from the `checked_write` contract.
...
// arch/riscv/cpu/mm.rs — I-8 callee-record (R-020)
impl Hart {
    fn checked_write(&mut self, addr: VirtAddr, size: usize,
                     value: Word, op: MemOp) -> XResult;
        // post-condition: on Ok, if op ∈ {Store, Amo}
        //   then self.last_store == Some((pa, size)).
        // Signature unchanged; pa is the local already returned by
        // access_bus (mm.rs:272) — no XResult<usize> widening.
    pub(super) fn store(&mut self, addr: VirtAddr, size: usize,
                        value: Word) -> XResult;       // unchanged body shape
    pub(super) fn amo_store(&mut self, addr: VirtAddr, size: usize,
                            value: Word) -> XResult;   // unchanged body shape
}
...
- **C-7** Plan body ≤ 700 lines (round-02 rebaseline; round-01's
  420 proved infeasible once Response Matrix + test table + I-8
  exposition were all included). Validated by `wc -l 03_PLAN.md`.
...
6. Rewire `CoreOps`: `pc` / `halted` / `halt_ret` → `self.current()`;
   ...
   `last_store` via `.take()`; advance cursor.
...
12. **I-8 callee-record (R-020 / TR-8 / R-018)**: inside
    `Hart::checked_write` at `mm.rs:271-276`, after the successful
    `self.bus.write(pa, size, value)` and *before* `Ok(())`, insert:
    ```rust
    if matches!(op, MemOp::Store | MemOp::Amo) {
        self.last_store = Some((pa, size));
    }
    ```
    `pa` is already in scope (local from `access_bus` at line 272)
    — no signature change, no caller-edit churn. The `op` gate is
    defensive: current callers are only `Hart::store` (Store) and
    `Hart::amo_store` (Amo), but the gate guards future Fetch /
    Load callers. Coverage: `store_op` (base.rs:71), `fstore_op`
    (float.rs:281), `sc_w` (atomic.rs:66), `sc_d` (atomic.rs:87)
    funnel through `Hart::store`; `amo_w` (atomic.rs:29), `amo_d`
    (atomic.rs:44), all 18 AMO dispatch arms funnel through
    `Hart::amo_store`; both funnel through `checked_write`. In
    `RVCore::step` after `step_one`, take `last_store` and if
    `Some`, call `invalidate_reservations_except`.
...
14. Rewrite `arch/riscv/device/intc/plic.rs`: delete `NUM_CTX`,
    ...
    bounds check uses `self.num_ctx`. Existing `setup()` test helper
    becomes `Plic::new(1, vec![irq.clone()])`. **All 14 existing
    PLIC `#[test]` functions** *(R-022)* — `priority_read_write`,
    `enable_per_context`, …, `reset_clears_state` (verified
    `grep -c '#\[test\]' plic.rs == 14`) — pass unchanged after the
    runtime-size reshape (V-IT-6 regression-block).
...
18. **`X_HARTS` env var (R-013 / R-023)**: in
    `xemu/xdb/src/main.rs::machine_config`, mirror the `X_DISK`
    `match` shape at `main.rs:45-53` exactly:
    ```rust
    ...
    ```
    Then thread `num_harts` into the `MachineConfig` build via
    `MachineConfig::with_harts(num_harts)` (or chained on the
    existing `with_disk` / default builder). No `cli.rs` edit; no
    clap attribute; no closure-with-inner-`?` (R-021 absorbed).
...
- **S0 → S1 (PR1)** `harts: Vec<Hart> (len 1)`; I-8 mm-layer hook
  live (callee-record inside `checked_write`) but no-op (no peer
  hart to invalidate); `mhartid` seeded by `Hart::new`. Guest-visible:
  identical (I-4).
...
- All **14 existing PLIC tests** *(R-022)* pass unchanged via V-IT-6
...
- **T-6 I-8 hook depth (R-011/TR-6)** (a) per-op at
  store_op/sc/amo/fstore — rejected, easy to miss future store ops;
  (b) mm-layer at `Hart::store` + `Hart::amo_store` — superseded by
  T-9; coverage is a property of memory semantics.
...
- **T-9 I-8 record site (R-020 / TR-8)** (a) caller-record after
  widening `checked_write` to `XResult<usize>` — signature churn +
  duplicated assignment in two callers; (b) callee-record inside
  `checked_write` gated on `op` — **chosen**, single site, no
  signature change, `pa` already in scope at `mm.rs:272`.
...
Regression block V-IT-6 *(R-019, R-022)*: **14 existing PLIC
`#[test]` functions** (`priority_read_write`, …, `reset_clears_state`;
verified `grep -c '#\[test\]' plic.rs == 14`) pass unchanged with
`Plic::new(1, vec![irq.clone()])`. Zero-regression gate, not a new
test.
...
| G-8 (PR2a PLIC reshape) | V-UT-10, V-IT-6 (14 existing PLIC tests), PR2a gate matrix (375-test count) |
| G-9 (PR2b SMP boot) | V-IT-5, V-E-4, V-F-5, V-F-6 |
| G-10 (cross-hart LR/SC via checked_write post-condition) | V-UT-11, V-UT-12, V-UT-13, V-UT-14 |
...
| C-7 (≤ 700-line budget) | `wc -l 03_PLAN.md` at plan-review |
...
| I-8 (cross-hart LR/SC via checked_write callee-record) | V-UT-11, V-UT-12, V-UT-13, V-UT-14 |
