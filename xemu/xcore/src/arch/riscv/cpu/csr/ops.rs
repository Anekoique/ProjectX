...
use crate::{arch::riscv::cpu::RVCore, config::Word, error::XResult};

impl RVCore {
    pub(in crate::arch::riscv) fn csr_read(&self, addr: u16) -> XResult<Word> {
    ...
    pub(in crate::arch::riscv) fn csr_write(&mut self, addr: u16, val: Word) -> XResult {
        let desc = find_desc(addr)
            .filter(|_| !Self::is_illegal_csr(addr))
