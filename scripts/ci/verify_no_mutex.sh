#!/usr/bin/env bash
#
# verify_no_mutex.sh — M-001 sentinel (R-002 in .ark/tasks/archive/legacy/perf-bus-fast-path/03_REVIEW.md).
#
...
if [[ $violations -gt 0 ]]; then
  ...
  echo "verify_no_mutex: see .ark/tasks/archive/legacy/perf-bus-fast-path/01_MASTER.md" >&2
  exit 1
fi
